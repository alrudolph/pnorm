...
class PostgresClientCounter(PostgresClient):
    ...
    def check_connections(self) -> int:
        ...
        assert len(self.create_connections) == 0 or self.create_connections[0]["create_connections_count"] == 1
        assert len(self.create_connections) == 0 or self.create_connections[-1]["create_connections_count"] == len(self.create_connections)
        
        for conn in self.create_connections:
            assert conn["create_connections_count"] - 1 == conn["close_connections_count"]
        
        assert len(self.close_connections) == 0 or self.close_connections[0]["close_connections_count"] == 1
        assert len(self.close_connections) == 0 or self.close_connections[-1]["close_connections_count"] == len(self.close_connections)
